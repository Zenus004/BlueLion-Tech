        ...
        <>
          <div className="space-y-3 relative">
            ...
            {state.items.map((item) => (
              <div key={item._id} className="rounded-xl bg-white/10 p-4">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button onClick={() => contactService.remove(item._id).then(load)} className="rounded-full bg-red-500/80 px-3 py-1 text-xs font-semibold">Delete</button>
                </div>
            ...
            {state.items.length === 0 && !state.loading && (
              <div className="text-center text-blue-200 py-8">No contacts found.</div>
            )}
          </div>
